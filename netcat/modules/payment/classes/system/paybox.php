...
class nc_payment_system_paybox extends nc_payment_system {
    ...
    // принимаемые валюты
    public $accepted_currencies = array('USD', 'EUR', 'RUR', 'RUB', 'KZT');
    ...
    public function execute_payment_request(nc_payment_invoice $invoice) {
        ...
        $script_url = "https://www." . $_SERVER['HTTP_HOST'] . nc_module_path('payment') . "callback.php?paySystem=nc_payment_system_paybox&invoice_id=" . $invoice->get_id();
        $arrFields = array(
        ...
        if ($this->get_setting('success_url')) {
            $arrFields['pg_success_url'] = $this->get_setting('success_url');
        }
        if ($this->get_setting('failure_url')) {
            $arrFields['pg_failure_url'] = $this->get_setting('failure_url');
        }
